...
const DISCORD_URL = "https://discord.gg/es6vcqA5zS"
...
export function DiscordFab() {
  ...
  useEffect(() => {
    const placeDefault = () => {
      const x = Math.max(window.innerWidth - 88, 12)
      const y = Math.max(window.innerHeight - 88, 12)
      setPos({ x, y })
    }
    placeDefault()
  }, [])
  ...
  useEffect(() => {
    const onMove = (e: PointerEvent) => {
      if (!dragging) return
      const nextX = startRef.current.px + (e.clientX - startRef.current.x)
      const nextY = startRef.current.py + (e.clientY - startRef.current.y)
      const clampedX = Math.min(Math.max(nextX, 12), window.innerWidth - 68)
      const clampedY = Math.min(Math.max(nextY, 12), window.innerHeight - 68)
  ...
  if (!enabled) {
    return (
      <button
        ...
        className="fixed bottom-5 right-5 z-[80] rounded-full border border-white/20 bg-black/70 px-3 py-1.5 text-xs text-white/80 backdrop-blur"
      >
        Show Discord
      </button>
    )
  ...
  return (
    ...
    >
      <button
        type="button"
        aria-label="Disable Discord bubble"
        onClick={() => setEnabled(false)}
        className="absolute -right-2 -top-2 z-[81] flex h-5 w-5 items-center justify-center rounded-full border border-white/30 bg-black/80 text-[10px] text-white/80"
      >
        ×
      </button>

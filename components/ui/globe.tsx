...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  useEffect(() => {
    ...
    globeRef.current
      ...
      .arcColor((e: number | Position) => (typeof e === "number" ? "#ffffff" : e.color))
...
export function WebGLRendererConfig() {
  ...
  useEffect(() => {
    gl.setPixelRatio(Math.min(window.devicePixelRatio || 1, 1.1));
...
export function World(props: WorldProps) {
  ...
  const rotateSpeed = globeConfig.autoRotateSpeed ?? 1;

  return (
    <Canvas
      scene={scene}
      camera={{ fov: 50, near: 180, far: 1800, position: [0, 0, cameraZ] }}
      frameloop={isAutoRotate ? "always" : "demand"}
      dpr={[1, 1.1]}
